...
function loadH1LineDescriptonMobile(){
...
// включити звук для відео
$(".songs-on").click(function() {
    myVideo.muted = false;
    $(this).css("display", "none");
    $(".songs-off").css("display", "block")
});
$(".songs-on-mobile").click(function() {
    myVideo2.muted = false;
    $(this).css("display", "none");
    $(".songs-off-mobile").css("display", "block")
});

// виключити звук для відео
$(".songs-off").click(function() {
    myVideo.muted = true;
    $(this).css("display", "none");
    $(".songs-on").css("display", "block")
});
$(".songs-off-mobile").click(function() {
    myVideo2.muted = true;
    $(this).css("display", "none");
    $(".songs-on-mobile").css("display", "block")
});

// функція для зупинення відео
function stop() {
   myVideo.pause();
...
// закриття відео та приховання кнопки закриття
function noneVideo() {
    $('.video-main #myVideo').addClass('none');
    $('.close-video').addClass('none');
    $("body").removeClass('body-over');
}

function noneVideoMobile() {
    $('.video-main #myVideo2').addClass('none');
    $('.close-video').addClass('none');
    $("body").removeClass('body-over');
}

// скік по кнопці для закриття блоку відео
$('.close-video').on('click', function (e){
    ...
    $("body").removeClass('body-over');
    loadH1LineDescriptonMobile();
    // myVideo.muted = true;
    stop();
});

$('.close-video-mobile').on('click', function (e){
    $('.video-main').addClass('video-none');
    // $('.video-main video').addClass('none');
    $('.index').addClass('index-auto');
    setTimeout(noneVideoMobile, 700);
    $("body").removeClass('body-over');
    loadH1LineDescriptonMobile();
    // myVideo.muted = true;
    stopMobile();
...
// функція для закрити блоку з відео після його завершення
function closeVideo() {
    $('.video-main').addClass('video-none');
    // $('.video-main video').addClass('none');
    $('.index').addClass('index-auto');
...
function closeVideoMobile() {
    ...
    // $('.video-main video').addClass('none');
    $('.index').addClass('index-auto');
    setTimeout(noneVideoMobile, 300);
    $("body").removeClass('body-over');
    loadH1LineDescriptonMobile();
    myVideo2.muted = true;
}

// закрити блок з відео після його завершення 31 сек
setTimeout(closeVideo, 31000);

if( window.innerWidth < 600 ) {
setTimeout(closeVideoMobile, 18000);
}
